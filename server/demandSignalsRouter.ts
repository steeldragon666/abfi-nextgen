...
export const demandSignalsRouter = router({
  ...
  submitResponse: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // Volume match
      if (
        input.proposedVolume >= signal.annualVolume * 0.9 &&
        input.proposedVolume <= signal.annualVolume * 1.1
      ) {
        matchScore += 20;
      ...
      if (signal.indicativePriceMin && signal.indicativePriceMax) {
        if (
          input.proposedPrice >= signal.indicativePriceMin &&
          input.proposedPrice <= signal.indicativePriceMax
        ) {
          matchScore += 20;

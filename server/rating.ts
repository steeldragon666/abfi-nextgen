...
export function calculateSustainabilityScore(
  ...
  if (
    feedstock.verificationLevel === "abfi_certified" ||
    feedstock.verificationLevel === "third_party_audited"
  ) {
    score += 20;
  } else {
    score += 10;
...
function calculateBambooQualityScore(parameters: any): number {
  ...
  const calorific = parameters.calorificValue?.value;
  if (calorific >= 4000)
    score += 30; // Premium P-Grade
  else if (calorific >= 3500) score += 25;
  else if (calorific >= 3000)
    score += 20; // Minimum P-Grade threshold
  else score += 10;
...
export function calculateReliabilityScore(transactions: Transaction[]): number {
  ...
  const ratedTransactions = completedTransactions.filter(
    t => t.buyerRating !== null
  );
  if (ratedTransactions.length > 0) {
    const avgRating =
      ratedTransactions.reduce((sum, t) => sum + (t.buyerRating || 0), 0) /
...
export function generateRatingImprovements(
  ...
  if (scores.sustainabilityScore < 80) {
    ...
    const hasPremiumCert = certificates.some(
      c =>
        c.status === "active" &&
        (c.type === "ISCC_EU" || c.type === "ISCC_PLUS" || c.type === "RSB")
    );

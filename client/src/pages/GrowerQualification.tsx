...
import { ArrowLeft, Award, FileText, TrendingUp, Shield, Calendar } from "lucide-react";
...
export default function GrowerQualification() {
  ...
  const { data: supplier, isLoading: supplierLoading } = trpc.suppliers.getById.useQuery(
    { id: parseInt(supplierId!) },
    { enabled: !!supplierId }
  );
  
  const createQualificationMutation = trpc.bankability.createGrowerQualification.useMutation({
    onSuccess: () => {
  ...
  const compositeScore = Math.round(
    (operatingHistoryScore + financialStrengthScore + landTenureScore + 
     productionCapacityScore + creditScore + insuranceScore) / 6
  );

  // Determine qualification level based on composite score
  const getQualificationLevel = (score: number): { level: string; name: string; color: string } => {
    if (score >= 85) return { level: "GQ1", name: "Premier", color: "text-green-600" };
    if (score >= 70) return { level: "GQ2", name: "Qualified", color: "text-blue-600" };
    if (score >= 55) return { level: "GQ3", name: "Developing", color: "text-yellow-600" };
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-sm font-medium">{operatingHistoryScore}/100</span>
                    </div>
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between items-center">
                      <Label className="font-semibold">Financial Strength</Label>
                      <span className="text-sm font-medium">{financialStrengthScore}/100</span>
                    </div>
                    <p className="text-sm text-muted-foreground">
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between items-center">
                      <Label className="font-semibold">Land Tenure Security</Label>
                      <span className="text-sm font-medium">{landTenureScore}/100</span>
                    </div>
                    <p className="text-sm text-muted-foreground">
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between items-center">
                      <Label className="font-semibold">Production Capacity</Label>
                      <span className="text-sm font-medium">{productionCapacityScore}/100</span>
                    </div>
                    <p className="text-sm text-muted-foreground">
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-sm font-medium">{creditScore}/100</span>
                    </div>
                    <p className="text-sm text-muted-foreground">
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between items-center">
                      <Label className="font-semibold">Insurance Coverage</Label>
                      <span className="text-sm font-medium">{insuranceScore}/100</span>
                    </div>
                    <p className="text-sm text-muted-foreground">
                  ...
                  <div className="flex gap-2 pt-4">
                    <Button
                      ...
                      className="flex-1"
                    >
                      {createQualificationMutation.isPending ? "Saving..." : "Save Qualification"}
                    </Button>
                    <Button
                      type="button"
                      variant="outline"
          ...
          <div className="space-y-6">
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="text-center py-6">
                  <div className={`text-6xl font-bold mb-2 ${qualification.color}`}>
                    {compositeScore}
                  </div>
                  <div className="text-sm text-muted-foreground mb-4">Composite Score</div>
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Operating History</span>
                    <span className="font-medium">{operatingHistoryScore}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Financial Strength</span>
                    <span className="font-medium">{financialStrengthScore}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{productionCapacityScore}</span>
                  </div>
                  <div className="flex justify-between">
            ...
            <Card>
              ...
              <CardContent className="space-y-3 text-sm">
                <div>
                  <div className="font-semibold text-green-600">GQ1 - Premier (85-100)</div>
                  <p className="text-muted-foreground">
                    Highest tier. Suitable for Tier 1 core agreements with minimal security requirements.
                  </p>
                </div>
                <div>
                  <div className="font-semibold text-blue-600">GQ2 - Qualified (70-84)</div>
                  <p className="text-muted-foreground">
                    Strong growers suitable for Tier 1 or Tier 2 with moderate security.
                  </p>
                </div>
                <div>
                  <div className="font-semibold text-yellow-600">GQ3 - Developing (55-69)</div>
                  <p className="text-muted-foreground">
                    Emerging growers suitable for Tier 2 or options with enhanced security.
                  </p>
                </div>
                <div>
                  <div className="font-semibold text-orange-600">GQ4 - Provisional (&lt;55)</div>
                  <p className="text-muted-foreground">
                    New or unproven growers. Require substantial security and monitoring.
                  </p>
                </div>
              </CardContent>
            </Card>

...
for (const company of supplierCompanies) {
  ...
  await db.insert(schema.suppliers).values({
    ...
    city:
      company.state === "QLD"
        ? ["Brisbane", "Townsville", "Mackay"][Math.floor(Math.random() * 3)]
        : company.state === "NSW"
          ? ["Sydney", "Newcastle", "Wagga Wagga"][
          ...
          : company.state === "VIC"
            ...
            : company.state === "WA"
              ? ["Perth", "Bunbury", "Albany"][Math.floor(Math.random() * 3)]
              : company.state === "SA"
                ? ["Adelaide", "Mount Gambier"][Math.floor(Math.random() * 2)]
                : company.state === "TAS"
                  ? ["Hobart", "Launceston"][Math.floor(Math.random() * 2)]
                  : company.state === "NT"
                    ? "Darwin"
                    : "Canberra",
    state: company.state,
...
console.log(
  `✅ Created ${supplierUsers.length} suppliers and ${buyerUsers.length} buyers\n`
);
...
const feedstockTemplates = [
  {
    category: "Sugar Cane Bagasse",
    description: "High-quality bagasse from sugar milling operations",
    minPrice: 45,
    maxPrice: 85,
  },
  {
    category: "Wheat Straw",
    description: "Agricultural residue from wheat harvesting",
    minPrice: 35,
    maxPrice: 65,
  },
  {
    category: "Canola Stubble",
    description: "Post-harvest canola crop residue",
    minPrice: 30,
    maxPrice: 55,
  },
  {
    category: "Wood Chips",
    description: "Sustainably sourced hardwood chips",
    minPrice: 55,
    maxPrice: 95,
  },
  {
    category: "Tallow (Category 1)",
    description: "Premium grade animal tallow",
    minPrice: 800,
    maxPrice: 1200,
  },
  {
    category: "Used Cooking Oil",
    description: "Collected and filtered UCO",
    minPrice: 600,
    maxPrice: 950,
  },
  {
    category: "Municipal Solid Waste",
    description: "Sorted organic fraction",
    minPrice: 25,
    maxPrice: 45,
  },
  {
    category: "Forestry Residue",
    description: "Logging and sawmill residues",
    minPrice: 40,
    maxPrice: 70,
  },
  {
    category: "Corn Stover",
    description: "Corn stalks and leaves post-harvest",
    minPrice: 35,
    maxPrice: 60,
  },
  {
    category: "Rice Husk",
    description: "Rice milling by-product",
    minPrice: 20,
    maxPrice: 40,
  },
];
...
for (const supplier of supplierUsers.slice(0, 12)) {
  // First 12 suppliers
  const numFeedstocks = Math.floor(2 + Math.random() * 4); // 2-5 feedstocks per supplier

  for (let i = 0; i < numFeedstocks; i++) {
    const template =
      feedstockTemplates[Math.floor(Math.random() * feedstockTemplates.length)];
    ...
    const abfiScore = calculateABFIScore(
      sustainability,
      carbon,
      quality,
      reliability
...
const allFeedstocks = await db
  .select()
  .from(schema.feedstocks)
  .limit(feedstockCount);

for (let i = 0; i < inquiryCount; i++) {
  const feedstock =
    allFeedstocks[Math.floor(Math.random() * allFeedstocks.length)];
